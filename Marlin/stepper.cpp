...
HAL_STEP_TIMER_ISR {
  ...
  if (!current_block) {
    ...
    if (current_block) {
     ...
     // SERIAL_ECHOLN(current_block->nominal_rate);
      #ifdef LASER
      counter_l = counter_x;
//      laser.dur = current_block->laser_duration;
     ...
     // SERIAL_ECHO("acc til: "); 
     // SERIAL_ECHO(current_block->accelerate_until);
     // SERIAL_ECHO(" deaccel after: ");
     // SERIAL_ECHOLN(current_block->decelerate_after);
	...
	#define STEP_END(axis, AXIS) _APPLY_STEP(AXIS)(_INVERT_STEP_PIN(AXIS),0)
      ...
      if (counter_l > 0) {
		...
		if (current_block->laser_mode == PULSED && current_block->laser_status == LASER_ON) { // Pulsed Firing Mode
			ulValue = (current_block->laser_raster_intensity_factor + Seven_factor) * LASER_PWM_MAX_DUTY_CYCLE;
